...
A1|p3
p4 <-DimPlot(sobj_ys_filtered_integrated, reduction = 'umap', group.by = 'LVL3',label.size = .05)
A3 <- DimPlot(sobj_ys_filtered, reduction = 'umap', group.by = 'LVL3', label.size = .05)
A3|p4

ElbowPlot(sobj_ys_filtered_integrated)
...
list_cluster <- sobj_ys_filtered_integrated@active.ident
...
FeaturePlot(sobj_ys_filtered_integrated, features = c('RUNX1','CD34','GATA2','GATA1','MITF','BATF','KIT','IL3',"FCER1A"))
...
sub_cds <- monocle3::cluster_cells(cds,
                                   method = 'louvain',
                                   res = 1e-6,
                                   louvain_iter = 1,
                                   verbose = T)
...
col_vector_origin <- c()
col_vector <-
  col_vector_origin[1:length(unique(as.character(pData(sub_cds)$Component)))]
names(col_vector) <- unique(as.character(pData(sub_cds)$Component))
options(repr.plot.width = 11)
options(repr.plot.height = 8)
plot_cell_clusters(sub_cds,
                   color_by = 'Tissue',
                   cell_size = 0.1,
                   show_group_id = T)  +
